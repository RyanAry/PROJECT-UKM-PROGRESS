<?php
    ...
    if (isset($_POST['edit'])) {
        ...
        $cek_data = "SELECT * FROM `partner_admin` WHERE `email` = '$email'";
        ...
        if ($cek->num_rows > 0) {
            ...
            $query_edit = "UPDATE `partner_admin` SET `nama`='$nama',`username`='$username',`password`='$password',`email`='$email',`role`='$role' WHERE `id`='$id';";
        ...
        $cek_data = "SELECT * FROM `partner_admin` WHERE `username` = '$username' OR `email` = '$email'";
        $cek = mysqli_query($db, $cek_data);
        ...
        if ($cek->num_rows > 0) {
            ...
            // insert data partner
            $query_add = "INSERT INTO `partner_admin`(`id_akomodasi`, `nama`, `username`, `password`, `email`, `role`) VALUES ('$id_akomodasi', '$nama', '$username', '$password', '$email', '$role')";
    ...
    $query_admin = "SELECT * FROM `partner_admin` WHERE id_akomodasi ='$data_admin[id_akomodasi]'";
